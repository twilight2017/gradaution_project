...
def run():
    ...
    weight_decay = 1e-6  # 调整优化器的参数
    alpha_weight = 0.01  # 自定义的衡量权重的参数
    beta_weight = 0.075
    gamma_weight = 0.25
    momentum = 0.9  # 调整优化器的参数
    ...
    def exp_lr_shedule(optimizer, step, init_lr=lr, lr_decay_step=lr_decay_step, step_decay_weight=step_decay_weight):
    ...
    if cuda:
        my_net = my_net.cuda()
        loss_classfication = loss_classfication.cuda()
...
# 定义源域和当前目标域
source_image_root = os.path.join('.', 'dataset', 'dataset1')
target_image_root = os.path.join('.', 'dataset', 'dataset2')
...
>>>>>>> parent of 92411a7... seventh commit
            my_net.zero_grad()
